...
def parse_args():
    ...
    parser.add_argument('--base_model_p', type=str)
    ...
    parser.add_argument('--use_lsq', type=str2bool, default=True)
...
def init_func(args, gpu_num):
    ...
    global p_frame_net
    p_frame_net = DMC(use_lsq=args.use_lsq)
    p_frame_net = p_frame_net.to(device)
    if args.base_model_p:
        print(f"Loading Base weights from {args.base_model_p}")
        base_state = get_state_dict(args.base_model_p)
        p_frame_net.load_state_dict(base_state, strict=False)
    if args.model_path_p:
        print(f"Loading LSQ weights from {args.model_path_p}")
        lsq_state = get_state_dict(args.model_path_p)
        p_frame_net.load_state_dict(lsq_state, strict=False)
    for name, module in p_frame_net.named_modules():
        ...
        for seq in config[ds_name]['sequences']:
            ...
            for rate_idx in range(rate_num):
                ...
                if use_multiprocessing:
                    ...
                    print(f"Processing {seq} QP {cur_args['qp_i']}...")
                    res = worker(cur_args)
                    results.append(res)

...
class LSQPrimitive(torch.autograd.Function):
    """
    LSQ Primitive: Quantizes input x with learnable step size s.
    Implements Straight-Through Estimator (STE) for gradients.
    """
    @staticmethod
    def forward(ctx, x, s, g, Qn, Qp):
        ...
        x_s = x / s
        # 2. Clip and Round
        x_q = torch.round(torch.clamp(x_s, Qn, Qp))
        # 3. De-scale (restore magnitude for QAT flow)
        x_hat = x_q * s
        return x_hat
    ...
    def backward(ctx, grad_output):
        ...
        # Gradients for x (pass-through where not clipped)
        grad_x = grad_output.clone()
        
        # Gradients for s (step size)
        x_s = x / s
        lower_bound = (x_s < Qn).float()
        upper_bound = (x_s > Qp).float()
        middle = 1.0 - lower_bound - upper_bound

        # LSQ gradient definition for s
        grad_s = (
            lower_bound * Qn +
            upper_bound * Qp +
            middle * (-x_s + torch.round(x_s))
        ) * grad_output * g

        # Sum gradients across batch/spatial dims for the single scalar s
        grad_s = grad_s.sum().unsqueeze(0)
...
class LSQConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, 
        ...
        # Learnable Step Sizes (Scalar per layer)
        self.s_w = nn.Parameter(torch.Tensor(1)) # Weight step size
        self.s_a = nn.Parameter(torch.Tensor(1)) # Activation step size
        
        self.is_initialized = False

    def forward(self, x):
        if not self.is_initialized:
            self._init_step_sizes(x)
            self.is_initialized = True

        # Scaling factors for gradient stability (from LSQ paper)
        g_w = 1.0 / math.sqrt(self.weight.numel() * self.Qp)
        g_a = 1.0 / math.sqrt(x.numel() * self.Qp)

        # Quantize Weights
        w_q = LSQPrimitive.apply(self.weight, self.s_w, g_w, self.Qn, self.Qp)
        
        # Quantize Input (Activation)
        x_q = LSQPrimitive.apply(x, self.s_a, g_a, self.Qn, self.Qp)

        return F.conv2d(x_q, w_q, self.bias, self.stride, 
                        self.padding, self.dilation, self.groups)

    def _init_step_sizes(self, x):
        """ Initialize s based on the mean absolute value of the first batch """
        with torch.no_grad():
            # Heuristic: 2 * mean(|v|) / sqrt(Qp)
            self.s_w.data.copy_(2 * self.weight.abs().mean() / math.sqrt(self.Qp))
            self.s_a.data.copy_(2 * x.abs().mean() / math.sqrt(self.Qp))

def replace_decoder_with_lsq(module):
    """
    Recursively replaces all nn.Conv2d layers with LSQConv2d 
    inside the given module.
    """
    for name, child in module.named_children():
        if isinstance(child, nn.Conv2d):
            # Create LSQ layer with identical config
            new_layer = LSQConv2d(
                child.in_channels,
                child.out_channels,
                child.kernel_size,
            ...
            # Transfer existing weights/bias
            ...
            # Recurse (handles DepthConvBlock, SubpelConv2x, Sequential, etc.)
            replace_decoder_with_lsq(child)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, ctx, quant_step,):
        is_lsq = hasattr(self.conv2, 's_w')
        if self.training or is_lsq or not CUSTOMIZED_CUDA_INFERENCE or not x.is_cuda:
...
class DMC(CompressionModel):
    def __init__(self, use_lsq=False): # Add flag
        ...
        if use_lsq:
            replace_decoder_with_lsq(self.decoder)
            replace_decoder_with_lsq(self.recon_generation_net)

...
def _load_mlcodec():
    global _MLCODEC
    if _MLCODEC is not None:
        return _MLCODEC

    if "MLCodec_extensions_cpp" in sys.modules:
        _MLCODEC = sys.modules["MLCodec_extensions_cpp"]
        return _MLCODEC

    import MLCodec_extensions_cpp
    _MLCODEC = MLCodec_extensions_cpp
    return _MLCODEC
...
class EntropyCoder():
    def __init__(self):
        ...
        mlcodec = _load_mlcodec()
        self.encoder = mlcodec.RansEncoder()
        self.decoder = mlcodec.RansDecoder()

    @staticmethod
    def pmf_to_quantized_cdf(pmf, precision=16):
        #from MLCodec_extensions_cpp import pmf_to_quantized_cdf as _pmf_to_cdf
        _pmf_to_cdf = mlcodec.pmf_to_quantized_cdf

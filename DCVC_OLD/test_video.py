...
def parse_args():
    ...
    parser.add_argument('--dump_calib', type=str2bool, default=False)
    parser.add_argument('--dump_dir', type=str, default="calib_dump")
    parser.add_argument('--dump_max', type=int, default=-1)
    parser.add_argument('--dump_only_p', type=str2bool, default=True)
    ...
    with torch.no_grad():
        while decoded_frame_number < frame_num:
            ...
            if header['nal_type'] == NalType.NAL_I:
                ...
                # ---------------- DUMP CALIB TENSORS ----------------
                if args.get('dump_calib', False):
                    if (not args.get('dump_only_p', True)) or (header['nal_type'] == NalType.NAL_P):
                        # optional limit
                        if not hasattr(run_one_point_with_stream, "_dump_count"):
                            run_one_point_with_stream._dump_count = 0
                        dump_max = args.get('dump_max', -1)
                        if dump_max < 0 or run_one_point_with_stream._dump_count < dump_max:
                            dump_pack = p_frame_net.get_last_decoder_dump()
                            if dump_pack is not None:
                                out_path = os.path.join(
                                    args['curr_dump_dir'],
                                    f"f{decoded_frame_number:05d}_p_qp{qp}.pt"
                                )
                                torch.save(dump_pack, out_path)
                                run_one_point_with_stream._dump_count += 1
                # ----------------------------------------------------
...
def worker(args):
    ...
    if args.get('dump_calib', False):
        dump_dir = os.path.join(args['dump_dir'], args['ds_name'], args['seq'], f"q{args['qp_i']}")
        args['curr_dump_dir'] = dump_dir
        create_folder(dump_dir, True)
